# Swyft Finance — Stage 2 Technical Project

Welcome to Stage 2. This is a **paid take-home project worth ₹5,000 INR**, sent via Wise once you submit. The code you write is yours to keep.

## Choose Your Project

Candidates must choose **ONE** of two projects:

| Option | Project | Description |
|--------|---------|-------------|
| A | [Quoting Calculator](./quoting-tool/) | Build a multi-lender loan quoting engine with precise financial calculations |
| B | [Bank Statement Tool](./bank-statement-tool/) | Build a bank statement analysis and visualization tool |

## Tech Stack

Both projects use the same stack. A Google Cloud project is provisioned for you.

- **Frontend:** Next.js (React) with TypeScript
- **Database:** Cloud SQL for PostgreSQL
- **Authentication:** Google OAuth via Google Identity Platform
- **File Storage:** Cloud Storage
- **Hosting:** Google Cloud — deploy to the project provisioned for you

## Evaluation Criteria

Both projects are evaluated on:

1. **Code Quality** (30%)
   - Clean, readable, maintainable code
   - Appropriate abstractions
   - Error handling
   - Testing approach

2. **Technical Accuracy** (30%)
   - Calculations match expected outputs exactly
   - Edge cases handled correctly
   - Data integrity maintained

3. **User Experience** (20%)
   - Intuitive interface
   - Responsive design
   - Helpful error messages
   - Performance

4. **Problem Solving & Communication** (20%)
   - Approach to ambiguous requirements
   - Documentation of assumptions
   - Communication during the project

## Timeline

All deadlines are **11:59 pm India Standard Time (IST)** on the date shown. These dates are fixed.

| Milestone | Date |
|-----------|------|
| Invitations sent | Fri 25 Sep |
| **Code due** | **Mon 28 Sep**, 11:59 pm IST |
| **Video due** | **Wed 30 Sep**, 11:59 pm IST |
| You hear back by | Fri 9 Oct |

This is a short window — roughly four days including a weekend — so scope your build accordingly. A focused, well-tested subset beats a broad, half-finished one.

- **Check-ins:** Daily async updates by email are encouraged
- **Questions:** Ask anytime via email

## Submission

1. Push code to a **private** GitHub repository
2. Add both `SauraPG72` and `s2dmad` as collaborators
3. Deploy to your provisioned **Google Cloud** project and provide the live URL
4. Include **tests**
5. Include a README covering:
   - Setup instructions
   - Architecture decisions
   - Known limitations
6. Submit a **10-minute video**, face on camera, by **Wed 30 Sep, 11:59 pm IST**, walking through your code and the decisions behind it:
   - Your database schema design decisions
   - Application architecture
   - Security implementation (RLS policies, auth)
   - Testing approach (E2E, unit tests)

## Getting Started

1. Read both project briefs carefully
2. Choose ONE project
3. Notify us which project you've chosen
4. Begin development

## Contacts

- Saura — Founder — saura@spgcapital.com.au
- Sid — Engineer — sid@spgcapital.com.au

If anything here is unclear, email either of us.

Good luck!
